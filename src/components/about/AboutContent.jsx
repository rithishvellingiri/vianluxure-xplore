      ...
      <div className="container mx-auto px-6 md:px-12 py-24 relative z-10 space-y-32">
        ...
        <section className="flex flex-col md:flex-row gap-16 items-center">
          ...
          <motion.div 
            ...
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            className="md:w-1/2 relative h-[500px] w-full rounded-sm overflow-hidden shadow-2xl border border-white/5"
          >
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-16">
          <motion.div 
            ...
            viewport={{ once: true }}
            className="bg-black/50 p-10 md:p-14 border border-white/10 glass rounded-sm shadow-xl"
          >
        ...
        {/* WHAT WE DO / BESPOKE */}
        <section className="relative -mx-6 md:-mx-12 px-6 md:px-12 py-24 bg-black/60 overflow-hidden shadow-2xl">
          <div className="absolute inset-0 z-0">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                ...
                <motion.div 
                  ...
                  className="bg-black/60 border border-white/10 glass p-10 text-center rounded-sm hover:-translate-y-2 transition-all duration-300 shadow-xl"
                >
                  ...
                  <h4 className="text-xl font-heading text-white mb-4 tracking-wide">{step.title}</h4>
                  <p className="text-sm text-gray-300 font-light leading-relaxed">{step.desc}</p>
                </motion.div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20">
            ...
            >
              <div>
                ...
                <h2 className="text-5xl md:text-6xl lg:text-7xl font-heading font-bold leading-[1.1] mb-8 text-[#111]">
                  Everything<br />You Need<br />to Know
                </h2>
                <div className="w-12 h-1 bg-[#d4af37] mb-8" />
              </div>
              <p className="text-[10px] font-semibold tracking-[0.25em] text-gray-400 uppercase mt-20">
            ...
            >
              {faqs.map((faq, index) => (
                <div key={index} className="border-b border-white/10 last:border-0 overflow-hidden">
                  <button 
                    onClick={() => setActiveFaq(activeFaq === index ? -1 : index)}
                    className="w-full py-6 flex items-center justify-between text-left focus:outline-none group"
                  >
                    <span className="text-xl md:text-[22px] text-white font-heading group-hover:text-primary transition-colors tracking-wide">{faq.q}</span>
                  ...
                  <AnimatePresence>
                    {activeFaq === index && (
                      ...
                      >
                        <p className="pb-8 text-gray-300 font-light leading-relaxed text-[15px] max-w-[90%]">
                          {faq.a}
                        </p>
                      </motion.div>
                    )}
